...
class Generator:
    def __init__(self):
        ...
        self.high_location_accuracy = 800
    ...
    def loadJSONData(self, json_file, date_range):
        ...
        with ProgressBar(max_value=len(data["locations"]), widgets=w) as pb:
            for i, loc in enumerate(data["locations"]):
                if "latitudeE7" not in loc or "longitudeE7" not in loc or "accuracy" not in loc:
                    continue
                if loc["accuracy"] > self.high_location_accuracy:
                    continue

                if "activity" in loc:
                    activity = loc["activity"][0]["activity"][0]["type"]
                    activity_confidence = loc["activity"][0]["activity"][0]["confidence"]
